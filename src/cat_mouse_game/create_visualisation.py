...
for topic, msgs, t in bag_mouse.read_messages(topics=['/mouse/base_pose_ground_truth']):
    point_list_mouse = np.append(point_list_mouse, [
                                 [msgs.pose.pose.position.x, msgs.pose.pose.position.y]], axis=0)
    mouse_time.append(t)
for topic, msgs, t in bag_cat.read_messages(topics=['/cat/base_pose_ground_truth']):
    point_list_cat = np.append(
        point_list_cat, [[msgs.pose.pose.position.x, msgs.pose.pose.position.y]], axis=0)
    cat_time.append(t)
