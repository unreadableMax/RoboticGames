...
def laser_callback_cat(current_laser_scan):
    ...
    pub=rospy.Publisher("cat/sonar",PointCloud,queue_size=1)
    output=PointCloud()
    header=Header()
...
def laser_callback_mouse(current_laser_scan):
    real_angles   = np.array([-90.0, -50.0, -30.0, -10.0, 10.0, 30.0, 50.0, 90.0])
    real_angles   = real_angles / 360.0 * 2 * np.pi
    opening_angle = 40.0/360.0*2*np.pi
    ...
    pub=rospy.Publisher("mouse/sonar",PointCloud,queue_size=1)
    output=PointCloud()
    header=Header()
    header.stamp=rospy.Time.now()
    header.frame_id="robot"
...
def convert_laser_sonar():
    ...
    rospy.Subscriber("/mouse/laser/scan",LaserScan,laser_callback_mouse)
    rospy.Subscriber("/cat/laser/scan",LaserScan,laser_callback_cat)
    rospy.spin()
