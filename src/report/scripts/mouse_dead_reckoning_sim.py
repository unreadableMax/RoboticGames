...
def noise_addition():
    rospy.Subscriber("/mouse/base_pose_ground_truth",
                     Odometry, calc_distance, float(sys.argv[1]))
    rospy.spin()


def calc_distance(data, var):
    pub = rospy.Publisher("/mouse/dead_reckoning", Pose, queue_size=1)
    fake_odom = data.pose.pose
    fake_odom.position.x = fake_odom.position.x + \
        np.random.normal(0, np.sqrt(var), 1)
    fake_odom.position.y = fake_odom.position.y + \
        np.random.normal(0, np.sqrt(var), 1)
    '''
    ...
    euler = euler_from_quaternion(
        [fake_odom.orientation.x, fake_odom.orientation.y, fake_odom.orientation.z, fake_odom.orientation.w])
    fake_odom.orientation.z = euler[2]+np.random.normal(0, np.sqrt(var), 1)
    pub.publish(fake_odom)


if __name__ == "__main__":
    rospy.init_node("mouse_dead_reckoning")
    try:
        noise_addition()
